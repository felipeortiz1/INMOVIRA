            ...
            <form action="{{ route('buscador.inmuebles') }}" method="GET" class="search-box">
                ...
                <div class="multi-select" id="multiSelect">
                    <div class="multi-select-display" id="selectedTypes">
                        @php
                            $selected = request('tipos') ? implode(', ', request('tipos')) : 'Tipo de inmueble';
                        @endphp
                        {{ $selected }}
                    </div>
                ...
                <!-- INPUT PARA MUNICIPIO O PALABRA CLAVE -->
                <input type="text" name="q" placeholder="Busca por municipio o palabra clave..."
                    value="{{ request('q') }}">
