                ...
                <form action="{{ route('usuario.update', $usuario->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="idMunicipio" id="idMunicipio" class="form-select">
                            ...
                            @foreach ($municipios as $m)
                                <option value="{{ $m->id }}" {{ $usuario->idMunicipio == $m->id ? 'selected' : '' }}>
                                    {{ $m->nombre }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="tipoUsuario" id="tipoUsuario" class="form-select" required>
                            <option value="persona" {{ $usuario->tipoUsuario == 'persona' ? 'selected' : '' }}>Persona
                            </option>
                            <option value="inmobiliaria" {{ $usuario->tipoUsuario == 'inmobiliaria' ? 'selected' : '' }}>
                                Inmobiliaria</option>
                        </select>
                    </div>
                    ...
                    <div class="d-flex justify-content-end mt-4">
                        ...
                        <a href="{{ route('usuario.index') }}" class="btn btn-outline-secondary rounded-pill px-4 me-2">
